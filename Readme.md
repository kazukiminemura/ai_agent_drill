...
2. お題にある入力例を渡す
3. 返り値の `type`、`content`、`messages` などを `print()` または `assert` で確認する
4. 合格条件を満たさない入力も試す
...
class FakeLLM:
    def chat(self, message: str) -> dict:
        if message == "こんにちは":
            return {"type": "final", "content": "こんにちは！"}
        return {"type": "final", "content": "対応していません。"}
...
出力で確認すること: `type` が `final` か、`content` に最終回答として表示できる文字列が入っているかを見ます。

回答の形:

```python
response = FakeLLM().chat("こんにちは")
assert response == {"type": "final", "content": "こんにちは！"}
print(response["content"])
```
...
出力で確認すること: `type` が `tool_call` か、`content` から tool 名と引数を取り出して次の関数呼び出しに使えるかを見ます。
...
2. 入力が `3 + 5 * 2 は？` なら、`type="tool_call"` の dict を返します。
...
出力で確認すること: `messages` の状態に応じて、次に tool を呼ぶべきか、`final` を返すべきかを `type` と `content` で判定できるかを見ます。
...
6. 最後に `{"type": "state", "content": state}` を返します。
...
2. `route()` は `{"type": "route", "content": {"target": ..., "reason": ...}}` を返す
3. `handoff(user_input)` の中で `decision = route(user_input)` を呼ぶ
4. `decision["content"]["target"]` を使って `answer` を作る
5. `handoff()` は `{"type": "final", "content": {"answer": ..., "log": decision}}` を返す
...
assert response["type"] == "guardrail"
...
出力で確認すること: `type` と `content` を見て、即実行、承認待ち、拒否のどの状態かを判定できるかを見ます。
...
```python
answer = {"type": "final", "content": {"answer": "返金は30日以内です。", "sources": [], "grounded": False}}
response = output_guardrail(answer)
assert response["content"]["passed"] is False
assert response["content"]["reason"]

answer = {"type": "final", "content": {"answer": "返金は30日以内です。", "sources": ["refund.md"], "grounded": True}}
response = output_guardrail(answer)
assert response["content"]["passed"] is True

answer = {"type": "final", "content": {"answer": "わかりません", "sources": [], "grounded": False}}
response = output_guardrail(answer)
assert response["content"]["passed"] is True
```
...
6. 問題なければ `{"type": "guardrail", "content": {"passed": True, "reason": ""}}` を返します。
...
出力で確認すること: JSONL の各行から `trace_id`、`timestamp`、`event_type`、event data を取り出し、実行の流れを後から追えるかを見ます。
...
3. `log(event_type, **data)` で event_type が空かどうかを検査します。
4. `trace_id`、`timestamp`、`type`、`content` を持つ event dict を作ります。
...
出力で確認すること: 通常回答、履歴、reset、exit などの結果が `type` と `content` から判定でき、CLI 側が表示や終了処理に使えるかを見ます。
...
4. 最後に `{"type": "final", "content": {"answer": answer}}` を返します。
...
assert trace_response["type"] == "trace"

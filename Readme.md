...
各 Drill は、文章で説明する問題ではなく、Python のコードで答える問題です。
README に書かれている `作るもの` の名前どおりに、関数やクラスを実装してください。

基本の答え方は次の形です。

1. `作るもの` に書かれた関数・クラスを作る
2. お題にある入力例を渡す
3. 返り値の `status`、`content`、`messages` などを `print()` または `assert` で確認する
4. 合格条件を満たさない入力も試す

たとえば `FakeLLM.chat()` を作る Drill なら、回答は次のように「実装 + 動作確認」まで書きます。

```python
class FakeLLM:
    def chat(self, message: str) -> dict:
        if message == "こんにちは":
            return {"status": "final", "content": "こんにちは！"}
        return {"status": "final", "content": "対応していません。"}


response = FakeLLM().chat("こんにちは")
...
```python
{"status": "final", "content": "答えです。"}
{"status": "tool_call", "content": {"tool_name": "calculator", "arguments": {"expression": "1 + 1"}}}
...
- `status`: LLM の返答の種類。`final` または `tool_call`
...
この教材では、`status` と `role` は別の種類の dict で使います。ひとつの dict に `status` と `role` を同時に入れません。

`status` は `FakeLLM.chat()` が返す dict の種類です。

```python
{"status": "tool_call", "content": {...}}
...
このドリルで主に練習するのは、自然な文章を書くことではなく、Agent のロジックを正しい順番で組むことです。
そのため `content` には、次の処理で必要になる情報や、テストで確認したい値を明示的に入れます。

- `tool_call` の `content`: どの tool を、どの引数で呼ぶか
- `tool` message の `content`: どの tool の結果か、成功したか失敗したか
- `final` の `content`: 最終的にユーザーへ返す答え、または後続の検査に必要な情報
- `state` や `guardrail` の `content`: 現在の状態、停止理由、ブロック理由など、分岐判断に必要な情報

`content` の中身が増えてきても、「その値を次のロジックが使うのか」「合格条件で確認するのか」を基準にしてください。
見た目の文章より、後からプログラムで取り出せる形になっていることを優先します。

`role` は `messages` に保存する会話履歴の発言者です。message dict には `role` を入れ、`status` は入れません。
...
1. LLM response: `FakeLLM.chat()` の返り値。`status` と `content` を持つ。
2. message: `messages` に保存する履歴。`role` と `content` を持つ。

OK:

```python
response = {"status": "tool_call", "content": {"tool_name": "calculator", "arguments": {"expression": "1 + 1"}}}
...
```python
message = {"role": "assistant", "status": "final", "content": "答えです。"}
message = {"role": "assistant", "content": {"status": "final", "content": "答えです。"}}
```

1つ目は、message dict に `status` が入っています。2つ目は、message の `content` に LLM response 全体を入れているため、`content["content"]` という分かりにくい形になります。
...
たとえば、次のコードは `status` を見ていません。`messages` の中から、`role` が `tool` の履歴だけを取り出しています。
...
出力で確認すること: `status` が `final` か、`content` に最終回答として表示できる文字列が入っているかを見ます。

回答の形:

```python
response = FakeLLM().chat("こんにちは")
assert response == {"status": "final", "content": "こんにちは！"}
print(response["content"])
```
...
```python
{"status": "final", "content": "こんにちは！"}
```

実装の流れ:

1. `FakeLLM` class を作ります。
2. `chat(self, message)` method を作ります。
3. `message == "こんにちは"` なら、上の `final` dict を返します。
4. それ以外は、同じ `final` の形で `対応していません。` を返します。
...
- `こんにちは` 以外の入力では、`status` は `final` のまま、`content` に `対応していません。` を返す
...
出力で確認すること: `status` が `tool_call` か、`content` から tool 名と引数を取り出して次の関数呼び出しに使えるかを見ます。

回答の形:

```python
response = FakeLLM().chat("3 + 5 * 2 は？")
call = response["content"]
assert response["status"] == "tool_call"
assert call["tool_name"] == "calculator"
assert call["arguments"] == {"expression": "3 + 5 * 2"}
```
...
1. `FakeLLM.chat(message)` で入力文字列を受け取ります。
2. 入力が `3 + 5 * 2 は？` なら、`status="tool_call"` の dict を返します。
3. `content["tool_name"]` には `calculator` を入れます。
4. `content["arguments"]` には、calculator に渡す `{"expression": "3 + 5 * 2"}` を入れます。
5. 対応していない入力では、`final` response を返します。
...
出力で確認すること: `content["arguments"]` から渡した値で関数が呼ばれ、計算結果だけを次の tool result に使える形で返せるかを見ます。

回答の形:

```python
call = {"tool_name": "calculator", "arguments": {"expression": "3 + 5 * 2"}}
result = calculator(**call["arguments"])
assert result == 13
```
...
1. `calculator(expression)` 関数を作ります。
2. まずは `"3 + 5 * 2"` だけを受け取り、`13` を返せば大丈夫です。
3. tool_call から `arguments = call["arguments"]` を取り出します。
4. `calculator(**arguments)` で dict の中身を関数に渡します。
5. 未対応の式では `ValueError` を出します。
...
出力で確認すること: tool の実行結果を `messages` に追加できるように、`role="tool"` と `content` 内の `tool_name` / `result` がそろっているかを見ます。

回答の形:

```python
message = make_message("calculator", 13)
assert message == {"role": "tool", "content": {"tool_name": "calculator", "result": 13}}
```
...
1. `make_message(tool_name, result)` 関数を作ります。
2. `tool_name` が空なら `ValueError` を出します。
3. `role` は必ず `"tool"` にします。
4. `content` の中に `tool_name` と `result` を入れて返します。
...
出力で確認すること: `messages` の状態に応じて、次に tool を呼ぶべきか、`final` を返すべきかを `status` と `content` で判定できるかを見ます。

回答の形:

```python
messages = [{"role": "user", "content": "3 + 5 * 2 は？"}]
first = FakeLLM().chat(messages)
assert first["status"] == "tool_call"

messages.append({"role": "tool", "content": {"tool_name": "calculator", "result": 13}})
...
1. `FakeLLM.chat(messages)` は list の会話履歴を受け取ります。
2. `messages` の中に `role == "tool"` の message があるか探します。
3. tool result がまだなければ、calculator の `tool_call` を返します。
4. tool result があれば、その `result` を使って `final` を返します。
5. tool result に `error` が入っていたら、失敗用の `final` を返します。
...
出力で確認すること: `run()` が Agent の途中処理を内部でつなぎ、最後に `final` の `content` だけをユーザー向け文字列として返せるかを見ます。
...
`messages` に入れる dict は `role` を持つ message、`FakeLLM.chat()` から返る dict は `status` を持つ response として扱います。
...
2. `FakeLLM.chat(messages)` を作る
   ...
   - tool message がまだなければ、`{"status": "tool_call", "content": ...}` を返す
...
3. `run(user_input: str) -> str` を作る
   ...
   - `response["status"] == "final"` なら `response["content"]` を返す
   - `response["status"] != "tool_call"` なら `ValueError` を出す
   ...
   - 最後の `response["status"]` が `final` でなければ `ValueError` を出す
   - `response["content"]` を返す

変数名は、dict の役割が分かる名前にしてください。

- `messages`: `role` を持つ message の list
- `response`: `status` を持つ LLM response
...
出力で確認すること: `call["tool_name"]` と `call["arguments"]` から tool を実行し、結果を `messages` に追加できる tool message として返せるかを見ます。
...
assert message["content"]["result"] == 13
```
...
1. `tools` dict に、tool 名と関数を登録します。
2. `run_tool(call, tools)` で `tool_name = call["tool_name"]` を取り出します。
3. `tool_name` が `tools` にない場合は `ValueError` を出します。
4. `tool = tools[tool_name]` で関数を取り出します。
5. `tool(**call["arguments"])` で実行し、tool message の dict に包んで返します。
...
assert run_result["status"] == "final"
...
1. `llm_response`: `FakeLLM.chat()` が返す制御用 dict。`status` と `content` を持ちます。
...
- `Runner.run()` の返り値は `{"status": "final", "content": {"answer": ..., "messages": ...}}`
...
- `response["status"] == "tool_call"` なら tool を実行して `messages` に足します。
- `response["status"] == "final"` ならそこで終了します。
...
- `messages` は Agent の作業ログです。中に入る各 dict は message なので、`status` ではなく `role` を持ちます。

assistant の tool_call も履歴に残す場合は、`llm_response` 全体ではなく `llm_response["content"]` だけを message の `content` に入れます。`status` は LLM response のキーなので、`role` を持つ message には入れません。
...
3. `llm_response["status"] == "tool_call"` なら、`llm_response["content"]` を `call` として取り出し、assistant message として `messages` に追加します。
4. `call["tool_name"]` で tool を探し、`call["arguments"]` を渡して実行します。
5. tool result を `role="tool"` の message として追加します。
6. `llm_response["status"] == "final"` なら assistant message を追加し、answer と messages を入れた `run_result` を返して loop を終えます。
...
出力で確認すること: 正常終了しない場合でも、途中までの `messages` と停止理由を確認でき、tool_call の回数が上限内に収まっているかを見ます。

回答の形:

```python
try:
    run("3 + 5 * 2 は？", max_turns=3)
except MaxTurnsExceededError as error:
    assert "max_turns exceeded" in str(error)
    assert len([m for m in error.messages if m["role"] == "tool"]) == 3
```
...
1. `MaxTurnsExceededError` class を作り、途中の `messages` を保持できるようにします。
2. `run()` の最初で `max_turns < 1` を検査します。
3. user message を `messages` に入れます。
4. `for _ in range(max_turns)` で、tool_call を最大回数だけ処理します。
5. loop 内で `final` が返ったら、その時点で `messages` を返します。
6. loop が終わっても `final` がなければ、`MaxTurnsExceededError("max_turns exceeded", messages)` を出します。
...
出力で確認すること: 計算できる入力では calculator に渡すための `tool_call` を返し、tool 不要または不正な入力では `final` やエラーとして分岐できるかを見ます。

回答の形:

```python
assert calculator("12 * 8") == 96
response = FakeLLM().chat("12 * 8 は？")
assert response["status"] == "tool_call"
assert response["content"]["arguments"]["expression"] == "12 * 8"
```
...
1. `validate_expression(expression)` で危険な文字列が含まれていないか確認します。
2. `ast.parse(expression, mode="eval")` で式を AST にします。
3. `evaluate(node)` で、許可した node だけを再帰的に計算します。
4. `calculator(expression)` は validate、parse、evaluate の順に呼びます。
5. `FakeLLM.chat(user_input)` は、計算式を含む入力なら calculator の `tool_call` を返します。
...
出力で確認すること: `FakeLLM.chat()` の `content` から検索 query を取り出し、`search_faq()` の戻り値を最終回答の材料として使えるかを見ます。
...
assert call["tool_name"] == "search_faq"
assert answer == "通常配送は3〜5営業日です。"
```
...
1. `FAQ` dict を用意します。
2. `search_faq(query)` で query が空かどうかを検査します。
3. query に含まれる keyword を `if` で調べ、対応する FAQ を返します。
4. どれにも当てはまらない場合は `見つかりませんでした。` を返します。
5. `FakeLLM.chat(user_input)` は、元の user_input を `arguments["query"]` に入れた tool_call を返します。
...
出力で確認すること: 成功時は `result`、失敗時は `error` を tool message の `content` に入れ、後続の LLM 判断に使える履歴になっているかを見ます。

回答の形:

```python
messages = run("Tokyo")
assert messages[-2]["content"]["result"]["weather"] == "sunny"
assert messages[-1]["content"] == "Tokyo は sunny です。"

messages = run("Unknown")
assert "error" in messages[-2]["content"]
assert messages[-1]["content"] == "天気を取得できませんでした。"
```
...
tool_call は `messages` に入れず、実行用の `response` として扱います。`status` は LLM response のキーなので、`role` を持つ message と同じ階層には置きません。
...
出力で確認すること: `content` に plan の構造化データが入り、必要な項目を validation で確認できるかを見ます。
...
`FakeLLM.chat()` の返り値は、[docs/message_contract.md](docs/message_contract.md) の LLM response に従い、必ず `status` と `content` を持つ dict にします。
...
1. `TaskPlan` dataclass を作ります。
2. `validate()` method で `steps` の数と `risk_level` を確認します。
3. `FakeLLM.chat(user_input)` は固定の structured output を返します。
4. `response["content"]` を `TaskPlan(**response["content"])` で dataclass に変換します。
5. `plan.validate()` を呼び、問題なければ plan を使える状態にします。
...
出力で確認すること: 壊れた JSON でも最終的に dict として扱える `content` へ戻し、repair できなかった場合の理由も確認できるかを見ます。

回答の形:

```python
broken = '{ goal: "調査する", steps: ["検索", "要約",], }'
data = parse_or_repair(broken)
assert data["goal"] == "調査する"
assert data["steps"] == ["検索", "要約"]
```
...
`FakeLLM.repair()` の返り値も、[docs/message_contract.md](docs/message_contract.md) に従い、必ず `status` と `content` を持つ dict にします。
...
1. `parse_or_repair(text, retry)` の中で `errors = []` を用意します。
2. まず `json.loads(text)` を試します。
3. 失敗したら `errors` にエラー文を追加します。
4. `FakeLLM.repair(text)` を呼び、返ってきた `content` を次の `text` にします。
5. retry 回数分だけ parse と repair を繰り返します。
6. 最後まで dict にできなければ、`ValueError` に errors の情報を含めて出します。
...
出力で確認すること: `recent()` の返り値が、そのまま prompt 用の `messages` として使える list になり、古い履歴が期待通り落ちるかを見ます。
...
出力で確認すること: `prompt_messages()` に summary と最新履歴が入り、LLM に渡す context が増えすぎない形になっているかを見ます。

回答の形:

```python
memory = SummaryMemory(keep_recent=4)
for i in range(12):
    memory.add("user", f"message {i}")

prompt = memory.prompt_messages()
...
出力で確認すること: 保存・読み出しの結果が `content` に入り、未保存の情報を作らずに分岐できるかを見ます。

回答の形:

```python
response = run("Pythonが好き")
assert response["status"] == "final"
...
response = run("好きな色は？")
assert response["content"] == "保存されていません。"
```
...
1. ファイル上部に `store = {}` を用意します。
2. `save_memory(key, value)` は `store[key] = value` で保存し、保存したことが分かる文字列を返します。
3. `read_memory(key)` は、key があれば値を返し、なければ `保存されていません。` を返します。
4. `run(user_input)` は、`Pythonが好き` なら `save_memory("favorite_language", "Python")` を呼びます。
5. `好きな言語は？` なら `read_memory("favorite_language")` を呼んで、その結果を `final` response に入れます。
6. 未対応の入力では `まだ対応していません。` を返します。
...
出力で確認すること: `content["answer"]` と `content["source"]` を見て、どの document を根拠に答えたかをプログラムで確認できるかを見ます。

回答の形:

```python
results = search_docs("返金期限は？")
assert results[0][0] == "refund.md"

response = answer("返金期限は？")
assert response["status"] == "final"
assert response["content"]["source"] == "refund.md"
```
...
出力で確認すること: `content["sources"]` に file と quote が入り、回答が実際の document に根拠づいているかを検査できるかを見ます。

回答の形:

```python
response = answer_with_source("返金できますか？")
validate_sources(response)
assert response["status"] == "final"
assert response["content"]["sources"][0]["file"] == "refund.md"
assert response["content"]["sources"][0]["quote"] in DOCS["refund.md"]
```
...
出力で確認すること: `content` に更新後の state が入り、plan、completed_steps、observations、final_answer の変化を追えるかを見ます。
...
assert response["content"]["observations"]
assert response["content"]["final_answer"]
```

state に goal、plan、completed_steps、observations、final_answer を持たせ、検索結果を観察して final を作ってください。返り値は `{"status": "state", "content": state}` にします。
...
1. `search_policy()` は固定文字列で返金ポリシーを返します。
2. `run(state)` で `plan` の最初の step を実行したことにします。
3. `search_policy()` を呼び、返り値を `observations` に追加します。
4. 実行した step を `completed_steps` に追加します。
5. observation があれば、短い `final_answer` を作ります。
6. 最後に `{"status": "state", "content": state}` を返します。
...
出力で確認すること: `content["reason"]` を見て、なぜループを止めたのかをプログラムで判定できるかを見ます。
...
response = run(actions)
assert response["status"] == "stop"
assert response["content"]["reason"] == "repeated action"
```
...
1. `max_steps < 1` なら `ValueError` を出します。
2. `actions` が空なら、`reason="finished"` の stop response を返します。
3. 先頭から `max_steps` 件まで action を確認します。
4. 直近3件が同じ dict なら、`reason="repeated action"` で止めます。
5. 繰り返しがなくても action 数が `max_steps` 以上なら、`reason="max_steps"` で止めます。
6. どの停止条件にも当てはまらなければ、`reason="finished"` を返します。
...
出力で確認すること: `content` に振り分け先、理由、handoff のログが入り、どの Agent に渡したかを確認できるかを見ます。

問い合わせを billing、tech_support、general_faq のどれかに振り分けてください。

この Drill では、まず `route()` が振り分け先を決めます。
返り値は次の形にしてください。
...
次に `handoff()` が `route()` を呼び、どの担当に渡したかを final response として返します。
`log` には `route()` の返り値をそのまま入れてください。

```python
{
    "status": "final",
    "content": {
        "answer": "billing に引き継ぎます。",
        "log": {
            "status": "route",
            "content": {
                "target": "billing",
...
1. `route(user_input)` で `target` と `reason` を決める
2. `route()` は `{"status": "route", "content": {"target": ..., "reason": ...}}` を返す
3. `handoff(user_input)` の中で `decision = route(user_input)` を呼ぶ
4. `decision["content"]["target"]` を使って `answer` を作る
5. `handoff()` は `{"status": "final", "content": {"answer": ..., "log": decision}}` を返す

動作確認例:

```python
result = handoff("請求書を再発行したい")
assert result["content"]["log"]["content"]["target"] == "billing"
assert result["content"]["answer"] == "billing に引き継ぎます。"
```
...
出力で確認すること: 各 agent 関数の出力が次の agent に渡り、`manager()` の `content` に最終レビュー結果が入るかを見ます。

回答の形:

```python
response = manager("返金ポリシーの記事を書いて")
assert response["status"] == "final"
assert response["content"]["answer"]
assert response["content"]["used_tools"] == ["research_agent", "writing_agent", "review_agent"]
```
...
1. `research_agent(topic)` は topic を受け取り、調査結果の文字列を返します。
2. `writing_agent(research)` は調査結果を受け取り、下書きの文字列を返します。
3. `review_agent(draft)` は下書きを受け取り、レビュー済みの文字列を返します。
4. `manager(user_input)` では、この3つを上から順に呼びます。
5. 呼んだ agent 名を `used_tools` list に入れます。
6. 最終的に review 結果を `content["answer"]` に入れた `final` response を返します。
...
出力で確認すること: `content["blocked"]` と `content["reason"]` を見て、tool 実行へ進めるかどうかを判定できるかを見ます。

回答の形:

```python
response = input_guardrail("rm -rf / を実行して")
assert response["status"] == "guardrail"
assert response["content"]["blocked"] is True
assert response["content"]["reason"] == "blocked keyword: rm -rf"

response = run("返金期限を教えて")
assert response["content"]["blocked"] is False
```
...
```python
{"status": "guardrail", "content": {"blocked": True, "reason": "dangerous input"}}
```

実装の流れ:

1. ブロックしたい keyword list を用意します。
2. `input_guardrail(user_input)` で、空文字なら `empty input` としてブロックします。
3. keyword が含まれていたら、`blocked=True` と理由を返します。
4. 危険でなければ、`blocked=False` と空の reason を返します。
5. `run(user_input)` では最初に guardrail を呼び、ブロックされたらその結果をそのまま返します。
6. ブロックされなければ、通常の `final` response を返します。
...
出力で確認すること: `status` と `content` を見て、即実行、承認待ち、拒否のどの状態かを判定できるかを見ます。

回答の形:

```python
response = request_tool("read_file", {"path": "memo.txt"})
assert response["status"] == "tool_result"

pending = request_tool("delete_file", {"path": "memo.txt"})
assert pending["status"] == "pending_approval"

rejected = approve(pending, allowed=False)
...
1. `request_tool()` で tool 名を見ます。
2. `read_file` はすぐに `tool_result` を返します。
3. `delete_file` と `send_email` は実行せず、`pending_approval` を返します。
4. `pending_approval` の `content` には、元の `tool_name` と `arguments` を残します。
5. `approve(pending, allowed=False)` なら `rejected` を返します。
6. `approve(pending, allowed=True)` なら、pending に残した tool 情報を使って `tool_result` を返します。

合格条件:

- `read_file` は `status="tool_result"` を返す
- `delete_file` と `send_email` は `status="pending_approval"` を返す
- reject されたら `status="rejected"` を返し、実行しない

条件を満たさない場合:

- 未知の tool は `status="rejected"` にして実行しない
- 承認待ちを reject した場合は `status="rejected"` を返し、tool 結果を作らない
...
出力で確認すること: `content["passed"]` や `content["reason"]` を見て、最終回答をユーザーに返してよいか判定できるかを見ます。

回答の形:

```python
answer = {"status": "final", "content": {"answer": "返金は30日以内です。", "sources": [], "grounded": False}}
response = output_guardrail(answer)
assert response["content"]["passed"] is False
assert response["content"]["reason"]

answer = {"status": "final", "content": {"answer": "返金は30日以内です。", "sources": ["refund.md"], "grounded": True}}
response = output_guardrail(answer)
assert response["content"]["passed"] is True

answer = {"status": "final", "content": {"answer": "わかりません", "sources": [], "grounded": False}}
response = output_guardrail(answer)
assert response["content"]["passed"] is True
```
...
`grounded` は、回答本文が sources の内容に基づいているかを表す真偽値です。`answer["content"]` の中に入れて使います。
...
`grounded=True` は「根拠に基づく回答なので許可できる」、`grounded=False` は「根拠が確認できない回答なので reject する」という意味です。ただし `answer` が `わかりません` の場合は、断定していないため sources や grounded がなくても許可して大丈夫です。

output guardrail は、受け取った answer dict を見て、返してよいかを判定します。reject するときは理由を `content["reason"]` に残します。

実装の流れ:

1. `answer["content"]` から `answer`、`sources`、`grounded` を取り出します。
2. 回答本文がない場合は reject します。
3. 回答本文が `わかりません` なら許可します。
4. sources が空なら reject します。
5. `grounded` が `False` なら reject します。
6. 問題なければ `{"status": "guardrail", "content": {"passed": True, "reason": ""}}` を返します。

合格条件:

- sources なしは reject
- `わかりません` は許可
- `grounded=False` は reject
- `content["reason"]` が残る
...
作るもの: `TraceLogger.log(event_status: str, **data) -> None`, `TraceLogger.jsonl() -> str`

出力で確認すること: JSONL の各行から `trace_id`、`timestamp`、`event_status`、event data を取り出し、実行の流れを後から追えるかを見ます。
...
lines = logger.jsonl().splitlines()
assert len(lines) == 3
assert logger.trace_id in lines[0]
...
各行に入れる event dict は、次の形にしてください。
...
`trace_id` は1回の実行を識別する ID です。`TraceLogger` を作ったときに1つだけ生成し、`user_input`、`tool_call`、`final` など、同じ実行中に記録するすべての event に同じ値を入れます。

`timestamp` は、その event を記録した時刻です。`log(...)` を呼ぶたびに現在時刻を作り、event dict に入れます。文字列にすると JSON にしやすいので、`datetime.now().isoformat()` のような形式で大丈夫です。

実装の流れ:

1. `TraceLogger.__init__()` で `trace_id` と `events` list を用意します。
2. `trace_id` は `uuid.uuid4()` などで作ります。
3. `log(event_status, **data)` で event_status が空かどうかを検査します。
4. `trace_id`、`timestamp`、`status`、`content` を持つ event dict を作ります。
5. event dict を `self.events` に追加します。
6. `jsonl()` では `self.events` を1行1 JSON に変換し、改行でつないで返します。

合格条件:

- すべての event に同じ trace_id がある
- すべての event に timestamp がある
- user_input、tool_call、final を記録できる

条件を満たさない場合:

- event_status が空なら `ValueError` を出す
- data は空でも記録できる
...
response = evaluate(dataset)
assert response["status"] == "eval"
assert response["content"]["accuracy"] == 1.0
assert response["content"]["rows"][0]["passed"] is True
```
...
1. `agent(user_input)` は固定の小さな Agent として、返金質問なら `search_docs` と回答を返します。
2. `evaluate(dataset)` の最初で、dataset が空なら `ValueError` を出します。
3. `rows = []` と `passed_count = 0` を用意します。
4. 各 case について `agent(case["input"])` を呼びます。
5. tool 名の一致と、期待回答が実回答に含まれるかを確認して `passed` を決めます。
6. case ごとの結果を row として `rows` に追加します。
7. `accuracy = passed_count / len(dataset)` を計算して `eval` response を返します。
...
出力で確認すること: 通常回答、履歴、reset、exit などの結果が `status` と `content` から判定でき、CLI 側が表示や終了処理に使えるかを見ます。
...
history = handle_command("/history")
assert history["status"] == "command"
assert "messages" in history["content"]

reset = handle_command("/reset")
assert reset["content"]["answer"] == "reset"
assert handle_command("/history")["content"]["messages"] == []
```
...
1. ファイルの上の方に `history = []` を用意します。
2. `chat(user_input)` では、まず user message を `history` に追加します。
3. そのあと簡単な回答文字列を作り、assistant message も `history` に追加します。
4. 最後に `{"status": "final", "content": {"answer": answer}}` を返します。
5. `handle_command(text)` では、`/exit`、`/history`、`/reset`、未知の `/` コマンド、通常入力の順に分岐します。

`history` に入れる message は、次のような dict で大丈夫です。
...
`/history` は履歴を読むだけです。`/reset` は `history.clear()` で履歴を空にします。`/exit` はプログラムを本当に終了させず、CLI 側が終了判断に使える dict を返すだけにします。

合格条件:

- 通常入力に `status="final"` で回答する
...
出力で確認すること: `/chat` の `content` から answer と trace_id を取り出せ、`/traces/{trace_id}` の `content` から events を確認できるかを見ます。

回答の形:

```python
response = chat({"user_input": "返金期限は？"})
assert response["status"] == "final"
trace_id = response["content"]["trace_id"]

trace_response = trace(trace_id)
assert trace_response["status"] == "trace"
assert isinstance(trace_response["content"]["events"], list)
```
...
1. `traces = {}` を用意して、trace_id から events を引けるようにします。
2. `agent(user_input)` は answer と trace_id を持つ `final` response を返します。
3. `agent()` の中で、user_input と final answer などの event を `traces[trace_id]` に保存します。
4. `chat(payload)` では `payload["user_input"]` を取り出し、なければ `ValueError` を出します。
5. `chat(payload)` は `agent(user_input)` の返り値をそのまま返します。
6. `trace(trace_id)` は `traces.get(trace_id, [])` を `events` として返します。
7. FastAPI が import できた場合だけ、`app = FastAPI()` と route 定義を作ります。

合格条件:

- `/chat` が `status="final"` で answer と trace_id を返す
- `/traces/{trace_id}` が `status="trace"` で events を返す
...
出力で確認すること: `load_messages()` の返り値が `messages` と同じ `role` / `content` の list になり、session_id ごとに履歴を復元できるかを見ます。

回答の形:

```python
save_message("session-1", "user", "こんにちは")
save_message("session-1", "assistant", "こんにちは！")
messages = load_messages("session-1")
assert messages == [
    {"role": "user", "content": "こんにちは"},
    {"role": "assistant", "content": "こんにちは！"},
]
assert load_messages("unknown") == []
```
...
1. `sqlite3.connect(":memory:")` で接続を作ります。
2. 起動時に `CREATE TABLE IF NOT EXISTS messages (...)` を実行します。
3. `save_message(session_id, role, content)` で空の session_id を拒否します。
4. `INSERT INTO messages (...) VALUES (...)` で1件保存します。
5. 保存後は `conn.commit()` を呼びます。
6. `load_messages(session_id)` では `SELECT role, content FROM messages WHERE session_id = ?` を実行します。
7. 取得した rows を `{"role": role, "content": content}` の list に変換して返します。

...
| --- | --- | --- | --- |
| 0.1 | FakeLLM が final を返す | `FakeLLM` | [drill_0_1_fake_llm_final.py](drill_0_1_fake_llm_final.py) |
| 0.2 | FakeLLM が tool_call を返す | `FakeLLM` | [drill_0_2_fake_llm_tool_call.py](drill_0_2_fake_llm_tool_call.py) |
| 0.3 | arguments で関数を呼ぶ | なし | [drill_0_3_tool_arguments.py](drill_0_3_tool_arguments.py) |
| 0.4 | tool result を message にする | なし | [drill_0_4_tool_result_message.py](drill_0_4_tool_result_message.py) |
| 0.5 | FakeLLM を2回呼ぶ | `FakeLLM` | [drill_0_5_fake_llm_two_turns.py](drill_0_5_fake_llm_two_turns.py) |
| 0.6 | 小さい Runner を関数で作る | `FakeLLM` | [drill_0_6_tiny_runner.py](drill_0_6_tiny_runner.py) |
| 1 | FakeLLM で Agent ループを書く | `Message`, `Tool`, `Agent`, `RunResult`, `FakeLLM`, `Runner` | [drill_1_agent_loop.py](drill_1_agent_loop.py) |
| 2 | max_turns で止める | `MaxTurnsExceededError`, `FakeLLM` | [drill_2_max_turns.py](drill_2_max_turns.py) |
| 3 | calculator tool | `FakeLLM` | [drill_3_calculator_tool.py](drill_3_calculator_tool.py) |
| 4 | dictionary search tool | `FakeLLM` | [drill_4_dictionary_search_tool.py](drill_4_dictionary_search_tool.py) |
| 5 | tool error handling | なし | [drill_5_tool_error_handling.py](drill_5_tool_error_handling.py) |
| 6 | TaskPlan を返す | `TaskPlan`, `FakeLLM` | [drill_6_task_plan.py](drill_6_task_plan.py) |
| 7 | 壊れた JSON を repair する | `FakeLLM` | [drill_7_json_repair.py](drill_7_json_repair.py) |
...
| 10 | key-value memory | なし | [drill_10_key_value_memory.py](drill_10_key_value_memory.py) |
| 11 | keyword search RAG | なし | [drill_11_keyword_search_rag.py](drill_11_keyword_search_rag.py) |
| 12 | citation 付き回答 | なし | [drill_12_citation_answer.py](drill_12_citation_answer.py) |
| 13 | plan -> act -> observe | なし | [drill_13_plan_act_observe.py](drill_13_plan_act_observe.py) |
| 14 | stop condition | なし | [drill_14_stop_condition.py](drill_14_stop_condition.py) |
| 15 | router agent | なし | [drill_15_router_agent.py](drill_15_router_agent.py) |
| 16 | agents as tools | なし | [drill_16_agents_as_tools.py](drill_16_agents_as_tools.py) |
| 17 | input guardrail | なし | [drill_17_input_guardrail.py](drill_17_input_guardrail.py) |
| 18 | tool approval | なし | [drill_18_tool_approval.py](drill_18_tool_approval.py) |
| 19 | output guardrail | なし | [drill_19_output_guardrail.py](drill_19_output_guardrail.py) |
| 20 | trace logger | `TraceLogger` | [drill_20_trace_logger.py](drill_20_trace_logger.py) |
| 21 | eval dataset | なし | [drill_21_eval_dataset.py](drill_21_eval_dataset.py) |
| 22 | CLI Agent | なし | [drill_22_cli_agent.py](drill_22_cli_agent.py) |
| 23 | FastAPI endpoint | なし | [drill_23_fastapi_endpoint.py](drill_23_fastapi_endpoint.py) |
| 24 | SQLite persistence | なし | [drill_24_sqlite_persistence.py](drill_24_sqlite_persistence.py) |
...
- 未保存の情報を聞かれたら `保存されていません。` を返す
- 対応していない入力では `まだ対応していません。` を返す
...
- score が 0 の場合は source を作らず、`見つかりませんでした。` を返す
...
- 根拠が見つからない場合は `answer` を `不明です。`、`sources` を空にする
...
- 危険入力では tool を実行せず、`blocked=True` と reason を返す
- 空の入力では `blocked=True` と `empty input` を返す
...
- 未知の tool は `rejected` にして実行しない
- 承認待ちを reject した場合は `rejected` を返し、tool 結果を作らない
...
- 不一致 case は `passed=False` として rows に残す
...
- 未知の `/` コマンドでは `unknown command` を返す
